...
export default async function Kalendar({ searchParams }: { searchParams: SP }) {
  ...
  const mine = { createdById: session!.sub, deletedAt: null };
  ...
  return (
    <div>
      <PageHeader title="Kalendar" subtitle={`Shu oyda ${monthRows.length} ta arizangiz`} />
                    ...
                    <ClickableRow key={c.id} href={`/arizalar/${c.id}`}>
                      <td className="px-4 py-3">
                        ...
                        <div className="mt-1 text-xs tabular-nums text-muted">
                          {formatSum(c.loanAmount.toString())} soʻm
                        </div>
                      </td>
